---
title: "SARIMA | Bus Passengers Complete"
format:
  html:
...
# Import dataset
ts_bus_passengers <- read_csv('../data/ts_bus_passengers_2.csv')
...
::: {.epigrafe}
In the search for optimal model parameters, a detailed analysis of the ACF and PACF plots was performed. The ACF plot showed autocorrelation at lag 4 and seasonal autocorrelation at lag 2, while the PACF plot showed autocorrelation at lag 2 and seasonal autocorrelation at lag 2. We chose the following parameters for the SARIMA model: 
...
To validate our selections, the auto.arima() function was used, resulting in a model with parameters (0,1,2)(0,0,2)[12]. This overall approach ensures that the SARIMA model is fine-tuned to recognize the underlying patterns in the time series data, thereby increasing predictive accuracy.
:::
...
::: {.epigrafe}
The evaluation of the model selection criteria shows a consistent preference for the model (0,1,2)(1,1,1)[12] by both AIC and AICc, while the BIC favors the model (0,1,0)(0,1,1)[12]. We can observe the parameter selection is the same as the Pre-COVID analysis. A comprehensive evaluation of their performance involved a meticulous analysis of diagnostic plots and auto.arima() results. Upon comparison, the same model, (0,1,0)(0,1,1)[12], emerges as a better choice. The standardized residuals show an apparently random pattern, the Q-Q plot shows a trend toward normality, and all Ljung box statistics fall slightly below the significance level, indicating minimal autocorrelation.
:::
...
::: {.epigrafe}
\begin{align}
(1 - \Theta_1 B^{12})y_t &= (1 - 0.9123 B^{12})\varepsilon_t \\
y_t &= \varepsilon_t + 0.9123 \varepsilon_{t-12}
\end{align}
:::
...
# Path to save plot
file_path <- "../images/6_bus_passengers_SARIMA.png"
