...
df_oil_price <-  df_oil_price_monthly %>% select("date", "adjusted")

# Save ts as a new file
write.csv(df_oil_price, '../data/df_oil_price_month.csv', row.names = FALSE)
...
In this case, since the information is in daily records, we have applied 12-month (1 year), 24-month (2 years), and 60-month (5 years) moving averages. A 12-month moving average smoothing provides a short to medium term perspective, reduces data volatility and noise, and represents the quarterly timeframe. As seen in the first chart, there is clearly no seasonality. The second plot shows a smoothing of the moving averages over a period of 24-month which helps to analyze the presence of economic shifts. The last graph, provides a long-term perspective by smoothing the moving averages over 60-month. We can see that there is no trend along the years and most of the analysis done in the previous section regarding economic crises, and external factors are still present in the fluctuations.
:::
...
# Path to save plot
file_path <- "../images/5_oil_price.png"
...
To find the autocorrelation of the series with its lagged values, we plot the ACF. Clearly, this time series is not stationary as there is a high positive autocorrelation for the first 24 lags.
...
To make the time series more stationary, we applied the detrending method to the fitted model of the oil price time series and viewed the results in the ACF plot. We also applied the first differencee to the original time series and plotted the results in two separate ACF plots. The ACF plot of the original time series is shown first to compare the different models.
:::
...
The first differencing show high autocorrelation in the first lag. The other lag values are very close to zero, therefore we can consider it stationary.
:::
...
# Path to save plot
file_path <- "../images/5_oil_price_stationary.png"

# Use ggsave to save the plot as a PNG image
ggsave(acf_3_oil_price, filename = file_path, width = 10, height = 5)
